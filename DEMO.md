...
| **"So data still goes to Anthropic — what's the privacy story?"** | "Three independent layers. One: raw DNA never leaves the browser through any channel we can observe. Two: only de-identified labels (gene, phenotype, drug name) ever go out, never rsIDs or genotypes. Three: every real query is mixed with 5 decoy queries to the same provider — same model, same prompt — which makes casual log inspection harder. We do NOT claim Anthropic literally can't tell — they can correlate by tool-use pattern and IP. We claim minimization plus a meaningfully bigger haystack. Honest framing matters here." |
| **"Only 17 drugs? CPIC has 119."** | "Correct — Phase 1 covers the 17 most clinically-actionable CPIC pairs with Strong-evidence guidance for genes consumer arrays can reliably call. We deliberately did NOT rush HLA-B (Stevens-Johnson on carbamazepine), G6PD, or DPYD (5-FU chemotherapy deaths) into a research prototype — those are highest-stakes and need real clinical review. Naming them on the Phase 2 roadmap honestly is better than half-shipping them tonight." |
| **"You wrote the literature-grounded test cases yourself — isn't that circular?"** | "Yes, the lit-grounded test is engineer-authored — both the cases and the grader live in the same repo. We treat that as a regression test, not an external validation. Our external tier is PGxQA from Keat et al., PSB 2025 — questions and grading authored by clinicians at UPenn and Stanford. We pass 5 of 6 in-scope partial matches and 4 of 4 out-of-scope refusal on that benchmark. The bio collaborator's curated mock-patient runner is a third tier — Rachel authored 5 synthetic patients with seeded variants and the CPIC phenotype each diplotype should produce; our parser + engine pipeline matched 19 of 19 assertions, including the deliberate 'Not determined' honesty case on patient 05 where coverage is partial." |
